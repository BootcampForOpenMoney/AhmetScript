...
// var kullanici = Number(prompt("çekmek istediğiniz miktarı girin"));

// if (kullanici <= hesabim ){
// console.log("girilen miktar para çekimi için yeterli");
// }
// else if (kullanici > hesabim){
//     console.log("girdiğiniz miktar hesabınızda olan miktardan daha fazla");
// }
...
function hesapDogrula(number) {
    var kullanıcı = Number(prompt("Lütfen hesap no girin"));
    if (kullanıcı == number){
        paraCek(hesapA, 2000);
    }
    else {
        console.log("hatalı hesap no ");
    }
}
function paraCek(hesap, miktar) {
    
  console.log(`Merhaba kardeşim ${hesap.ad}`);
  if (hesap.bakiye >= miktar) {
    hesap.bakiye = hesap.bakiye - miktar;
    console.log(
      `${hesap.hesapNo} numaralı hesabınızdan çekim onaylanmıştır.Hesabınızda kalan miktar ${hesap.bakiye}TL dir `
    );
  } else {
    var toplam = hesap.bakiye + hesap.ekHesap;
    if (toplam >= miktar) {
      if (confirm("ek hesaptaki miktarınızı kullanmak ister misiniz")) {
        if (miktar <= toplam) {
          hesap.ekHesap = toplam - miktar;
          hesap.bakiye = 0;
          console.log(`${hesap.hesapNo} numaralı hesabınızdan çekim onaylanmıştır.Hesabınızda kalan miktar ${hesap.bakiye}TL ek hesabınızda ${hesap.ekHesap}TL kalmıştır.`);
        } 
      }else {
          console.log(`ek hesabınızı kullanmadan, çekmek istediğiniz ${miktar}TLyi çekmek için lütfen hesabınıza ${toplam - miktar} TL yatırınız.`);
      }
    }else {
        console.log("hesabınızda yeterli miktarda para bulunmamaktadır.");
    }
  }
}


hesapDogrula(hesapA.hesapNo);